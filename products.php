        ...
        <div class="row g-4 justify-content-between align-items-center mb-5">
            <div class="col-lg-8" data-aos="fade-right">
                <div class="d-flex flex-wrap gap-2">
                    <a href="/DonaMart/products.php" class="btn btn-sm <?= empty($cat_slug) ? 'btn-accent' : 'btn-outline-primary-custom' ?> rounded-pill px-4 py-2 font-weight-bold">All Products</a>
                    <?php foreach ($categories as $cat): ?>
                        <a href="/DonaMart/products.php?category=<?= $cat['slug'] ?><?= !empty($search) ? '&search='.urlencode($search) : '' ?>"
                           class="btn btn-sm <?= ($cat_slug === $cat['slug']) ? 'btn-accent' : 'btn-outline-primary-custom' ?> rounded-pill px-4 py-2 font-weight-bold">
            ...
            <div class="col-lg-4 col-md-8 ms-auto" data-aos="fade-left">
                <form action="/DonaMart/products.php" method="GET" class="search-form">
                    ...
                    <div class="input-group bg-white shadow-sm rounded-pill overflow-hidden border border-light">
                        <input type="text" name="search" class="form-control border-0 px-4" placeholder="Search products..." value="<?= htmlspecialchars($search) ?>">
                ...
                <?php foreach ($products as $p): ?>
                    <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="100">
                        <div class="product-card">
                            <!-- FIXED: image wrapper now has consistent height + object-fit -->
                            <div class="product-img-wrapper">
                                ...
                                <a href="/DonaMart/product-details.php?slug=<?= urlencode($p['slug']) ?>">
                                    <img src="/DonaMart/uploads/<?= htmlspecialchars($p['image']) ?>"
                                         ...
                                         loading="lazy"
                                         onerror="this.onerror=null;this.src='/DonaMart/assets/images/placeholder.png';">
                                </a>
                            </div>
                            <div class="product-details">
                                ...
                                <p class="text-muted text-sm mb-3"><?= htmlspecialchars($p['description']) ?></p>

                                <table class="table product-spec-table">
                                    <tr><td>Available Sizes:</td><td><?= htmlspecialchars($p['sizes']) ?></td></tr>
                                    <tr><td>Material:</td><td><?= htmlspecialchars($p['material']) ?></td></tr>
                                    <tr><td>Min. Order Qty:</td><td><?= number_format($p['moq']) ?> pcs</td></tr>
                                </table>

                                <div class="product-actions d-grid gap-2">
                                    <a href="/DonaMart/bulk-order.php?product=<?= urlencode($p['name']) ?>" class="btn btn-accent rounded-pill font-weight-bold">Enquire Bulk Price</a>
                                    <a href="https://wa.me/918874812003?text=Hi%20DonaMart,%20I'm%20interested%20in:%20<?= urlencode($p['name']) ?>" target="_blank" class="btn btn-whatsapp rounded-pill font-weight-bold">
